...
- Campos del modal DCANP en orden: Nombre y apellido → Teléfono → Ciudad → Dirección exacta (`id="d-calle"`, opcional) → Nota del pedido (opcional) → Envío express → Necesito factura.
- El campo Ciudad en modales DCANP usa siempre el selector con autocomplete (`dcanp-cities.js?v=1`): muestra "Ciudad (Departamento)" en el dropdown, guarda solo el nombre de la ciudad. No usar `<input type="text">` libre sin el selector.
- El teléfono se normaliza en `dcanp-lead.js` con `normalizarTelefono()` — quita +595/595 y deja el número local (0XXXXXXXX).
