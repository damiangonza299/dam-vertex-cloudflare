/* =========================================================
   _alert-engine.js — Motor de alertas compartido
   Importado por: alerts.js, send-alerts.js
   NO es una ruta de Cloudflare Pages (prefijo _).

   Tipos de alerta:
     creative_dead      — creativo muerto (vencidos altos, compras nulas)
     creative_scalable  — creativo ganador (purchase_rate + score altos)
     garbage_risk       — tráfico basura (por creativo o ciudad)
     winning_city       — ciudad con conversión > 40%
     buyer_type_increase— compradores premium aumentaron vs semana anterior
     conversion_drop    — caída global ≥ 40% de compras vs semana anterior

   Muestras mínimas: 10 leads, 3 compras, 3 días de datos.
...
export async function generateAlerts(DB) {
  ...
  const [creativeLeads, creativeLQ, cityData, buyerTrend, convTrend] = await Promise.all([
    fetchCreativeLeads(DB, since30),
    fetchCreativeLQ(DB, since30),
    fetchCityData(DB, since30),
    fetchBuyerTrend(DB, since7, since14),
    fetchConversionTrend(DB, since7, since14),
  ]);
  ...
  for (const cr of creativeLeads) {
    ...
    // 3. Riesgo tráfico basura (por creativo — si no es ya creative_dead)
    if (
      hasData &&
      dead_lead_rate >= 0.5 &&
      total_leads >= 15 &&
      !alerts.some(a => a.tipo === 'creative_dead' && a.evidencia?.ad_id === cr.ad_id)
    ) {
      alerts.push({
        tipo:                'garbage_risk',
        severidad:           'alta',
  ...
  for (const city of cityData) {
    ...
    // 3. Riesgo tráfico basura (por ciudad)
  ...
  // 5. Buyer type premium aumentó vs semana anterior
  ...
  // 6. Caída fuerte de conversión global (compras últimos 7d vs 7d anteriores)
  const recentPurchased = Number(convTrend?.recent_purchased || 0);
  const prevPurchased   = Number(convTrend?.prev_purchased   || 0);

  if (prevPurchased >= MIN_PURCHASES && recentPurchased < prevPurchased * 0.6) {
    const delta = Math.round(((recentPurchased - prevPurchased) / prevPurchased) * 100);
    alerts.push({
      tipo:                'conversion_drop',
      severidad:           'alta',
      titulo:              `Caída de conversión: ${Math.abs(delta)}% menos compras`,
      explicacion:         `Las compras confirmadas cayeron ${Math.abs(delta)}% vs la semana anterior (${recentPurchased} esta semana vs ${prevPurchased} la semana pasada).`,
      evidencia: {
        recent_7d:  recentPurchased,
        prev_7d:    prevPurchased,
        delta_pct:  delta,
      },
      accion_sugerida:     'Revisar creativos activos, tasa de cierre en admin panel y si hay problemas operativos de stock o delivery.',
      requiere_aprobacion: false,
    });
  }
...
async function fetchConversionTrend(DB, since7, since14) {
  try {
    const { results } = await DB.prepare(`
      SELECT
        SUM(CASE WHEN purchased_at >= ?                       THEN 1 ELSE 0 END) AS recent_purchased,
        SUM(CASE WHEN purchased_at >= ? AND purchased_at < ?  THEN 1 ELSE 0 END) AS prev_purchased
      FROM leads
